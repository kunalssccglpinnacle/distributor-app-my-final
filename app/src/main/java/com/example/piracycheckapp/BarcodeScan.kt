            ...
            override fun onResponse(call: Call<CountResponse>, response: Response<CountResponse>) {
                if (response.isSuccessful) {
                    ...
                    val message = if (count == 1) {
                        "Barcode scanned $count time so it is fair"
                    } else {
                    ...
                    val spannable = SpannableString(message)
                    if (count != 1) {
                        spannable.setSpan(ForegroundColorSpan(Color.RED), message.indexOf("doubtful"), message.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
                    }
